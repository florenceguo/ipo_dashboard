...
async function loadExcelData() {
    try {
        // 显示加载进度
        updateLoadingProgress('正在加载数据文件...');
        
        // 检查浏览器缓存
        const cachedData = localStorage.getItem('ipo_dashboard_data');
        const cacheTime = localStorage.getItem('ipo_dashboard_cache_time');
        const now = new Date().getTime();
        
        // 如果有缓存且未过期（1小时），直接使用缓存
        ...
        // 如果没有缓存或已过期，重新加载
        // 优先尝试加载优化版本
        const response = await fetch(`excel_data_optimized.json`);
        if (!response.ok) {
        ...
        // 缓存数据到本地存储
        localStorage.setItem('ipo_dashboard_data', JSON.stringify(excelData));
        localStorage.setItem('ipo_dashboard_cache_time', now.toString());
